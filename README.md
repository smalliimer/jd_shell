...
内有多个环节选择，可退出！
```shell
bash <(curl -sSL https://cdn.jsdelivr.net/gh/lan-tianxiang/jd_shell/install_scripts/linux_install_jd.sh)
```

### 2. Docker 一键部署单个容器：

```shell
bash <(curl -sSL https://cdn.jsdelivr.net/gh/lan-tianxiang/jd_shell/install_scripts/docker_install_jd.sh)
```
